...
while True:
    ...
    if bbox is not None:
        # Calculer l'angle entre le code QR et la caméra
        points = np.array(bbox[0], np.int32)
        x1, y1 = points[0]
        x2, y2 = points[1]
        x3, y3 = points[2]
        angle = round(math.atan2(y2-y1, x2-x1) - math.atan2(y3-y1, x3-x1) + math.pi, 2) * 180/math.pi
        angle = round(math.atan2(y2-y1, x2-x1) + math.atan2(y3-y1, x3-x1) - math.pi/2, 2) * 180/math.pi

        print(f"L'angle entre le code QR et la caméra est {angle} degrés.")

    # finding width of QR code width in the frame 
    codeWidth= DetectQRcode(img)
    #print(barcodes)
    # check if there is a QRCode in the image
    # if data and barcodes:
    #     #cv2.waitKey(0)
    ...
    #     if degree == 90:
    #         degree = 0
    #     print("Le degree est : " + str(degree))
    #     a=data
    #     fin(data)
    #     data=None
