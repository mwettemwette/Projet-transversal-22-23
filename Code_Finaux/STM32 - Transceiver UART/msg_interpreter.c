	...
	if (interpreter_running == 0)
	{
		...
		if (Size > 0)
		{
			if (strncmp("rpi", dev, 3) == 0)
			{
				...
				{
					//sscanf(s, "DIR_INIT:%d:%d", a, b);
				}
				/************** Command to open a specific cell of the letter box *****************/
				if (strncmp("OPEN_CELL:", (char *)InterpretBuf, 10) == 0)
				{
					int cell_number = 0;
				...
				if (strncmp("CLOSE_CELL:", (char *)InterpretBuf, 11) == 0)
				{
					int cell_number = 0;
					char cell_number_str[10];

					// Convert to integer the number sent alongside the command
					strncpy(cell_number_str, (char *)InterpretBuf+11, Size-11);
					cell_number = atoi(cell_number_str);
...
//				}
				...
				else if (strncmp("LIDAR:", (char *)InterpretBuf, 6) == 0)
				{
					sscanf((char *)InterpretBuf, "LIDAR:%d:%d:%d:%d:%d:%d:%d", &a, &b, &c, &d, &e, &f, &g);
				}
